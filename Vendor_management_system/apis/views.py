...
from vendor_models.models import Vendor,PurchaseOrder,Item,Buyer,HistoricalPerformance
        ...
        try:
            ...
            if purchase_order.buyer.user == request.user:
                if purchase_order.status == 'pending':
                    ...
                    purchase_order.save()
                    print(f" CancelPurchaseOrderView signals should trigger ---------------------------------")
                    return Response(
                        {
                            'responseCode': status.HTTP_200_OK,
        ...
        try:
            ...
            if quality_rating is not None:
                ...
                if 0 <= quality_rating <= 5:
                    ...
                    purchase_order.save()
                    print(f"RatePurchaseOrderView signals should trigger--------------------")
                    return Response(
                        {
                            'responseCode': status.HTTP_200_OK,
...
class CompletePurchaseOrderView(APIView):
    ...
    def post(self, request, purchase_order_id):
        try:
            ...
            purchase_order.save()
            print(f"CompletePurchaseOrderView signal should trigger---------------------------------------")
...
class AcknowledgePurchaseOrderView(APIView):
    ...
    def post(self, request, purchase_order_id):
        try:
            ...
            purchase_order.save()
            print(f"AcknowledgePurchaseOrderView signal should trigger---------------------------------------")
            return Response(
                {
                    'responseCode': status.HTTP_200_OK,
...
class PurchaseOrderListView(APIView):
    ...
    def get(self, request):
        try:
            vendor = request.user.vendor 
            buyer_name = request.query_params.get('buyer_name', '')
            purchase_orders = PurchaseOrder.objects.filter(vendor=vendor).order_by('id')
            if buyer_name:
